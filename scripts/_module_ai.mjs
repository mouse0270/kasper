...
export class MODULE {
	...
	static TITLE = 'Module Management+';
	...
	static log = (logLevel, ...args) => {
		...
		if (this.isDevMode === false) return false;
		...
		const logLevelIndex = logLevels.findIndex((level) => level.title.toLowerCase() === logLevel.toLowerCase());
		...
		if (logLevelIndex >= this.isDevMode) return false;
...
Hooks.once('devModeReady', ({ registerPackageDebugFlag }) => {
	...
	// Log Dev Mode Ready
	MODULE.log('INFO', 'DevMode Ready');
});

// Update Module Title with localized string once game.i18n is ready
Hooks.once('i18nInit', () => MODULE.TITLE = MODULE.localize('title'));