			...
			<ul>
				<li v-for="(faction, fIdx) in reputation.factions" :id="faction.uuid">
					<div class="form-group" :style="store.getStyle(reputation, faction, $el)">
						<label for="faction" :data-tooltip="faction.name.length > 20 ? faction.name : null" :contenteditable="(faction?.docUuid?.startsWith(`${store.ID}.`) ?? game.user.isGM) && game.user.isGM" @blur="store.onFactionNameChange(reputation, faction, $event, store)">{{faction.name}}</label>
